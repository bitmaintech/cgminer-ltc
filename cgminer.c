...
static struct opt_table opt_config_table[] =
{
    ...
    set_int_0_to_9999,opt_show_intval, &opt_bitmain_L3_freq,
    "Set frequency"),
    OPT_WITH_ARG("--bitmain-core-temp",
    set_int_0_to_100,opt_show_intval, &opt_bitmain_L3_core_temp,
    "Select core temp"),
    OPT_WITH_ARG("--bitmain-voltage",
    set_int_0_to_9999,opt_show_intval, &opt_bitmain_L3_voltage,
    "Set voltage"),
#endif
...
static void __kill_work(void)
{
    ...
    forcelog(LOG_DEBUG, "Received kill message");
...
static void *submit_work_thread(void *userdata)
{
    ...
    while (!submit_upstream_work(work, ce->curl, resubmit))
    {
        if (opt_lowmem)
        {
            applog(LOG_DEBUG, "Pool %d share being discarded to minimise memory cache", pool->pool_no);
            break;
        }
        resubmit = true;
        if (stale_work(work, true))
        {
            applog(LOG_DEBUG, "Pool %d share became stale while retrying submit, discarding", pool->pool_no);
        ...
        applog(LOG_DEBUG, "json_rpc_call failed on submit_work, retrying");
    }
...
void pool_died(struct pool *pool)
{
    if (!pool_tset(pool, &pool->idle))
    {
        ...
        if (pool == current_pool())
        {
            ...
            applog(LOG_DEBUG, "Pool %d %s failed to return work", pool->pool_no, pool->rpc_url);
    }
}
...
static void rebuild_hash(struct work *work)
{
    ...
    if (opt_scrypt)
    {
        ...
        // tmp = (unsigned char*)(work->data+76);
...
static void set_curblock(char *hexstr, unsigned char *bedata)
{
    ...
    applog(LOG_DEBUG, "New block: %s... diff %s", current_hash, block_diff);
}
...
static void hashmeter(int thr_id, uint64_t hashes_done)
{
    ...
    if (thr_id >= 0)
    {
        ...
        if (/*want_per_device_stats && showlog*/1)
        {
            char logline[256];
    ...
    {
        applog(LOG_ERR, "cgminer time error total_secs = %f last_total_secs = %f", total_secs, last_total_secs);
        mutex_unlock(&hash_lock);
        zero_stats();
        mutex_lock(&hash_lock);
    }
...
static bool parse_stratum_response(struct pool *pool, char *s)
{
    ...
    if (json_is_null(id_val) || !id_val)
    {
        ...
        applog(LOG_DEBUG,"JSON-RPC non method decode failed: %s", ss);
...
void clear_pool_work(struct pool *pool)
{
    ...
    if (cleared)
        applog(LOG_DEBUG, "Cleared %d work items due to stratum disconnect on pool %d", cleared, pool->pool_no);
}
...
void stratum_resumed(struct pool *pool)
{
    ...
    {
        applog(LOG_DEBUG, "Stratum connection to pool %d resumed", pool->pool_no);
        pool_resus(pool);
    }
}
...
static void *stratum_rthread(void *userdata)
{
    ...
    while (42)
    {
        ...
        if (!s)
        {
            applog(LOG_DEBUG, "Stratum connection to pool %d interrupted", pool->pool_no);
            pool->getfail_occasions++;
            total_go++;
        ...
        if (!parse_method(pool, s) && !parse_stratum_response(pool, s))
            applog(LOG_DEBUG, "Unknown stratum msg: %s", s);
...
static void *stratum_sthread(void *userdata)
{
    ...
    while (42)
    {
        ...
        *nonce2_64 = htole64(work->nonce2);
        ...
        {
            applog(LOG_DEBUG, "Filtering duplicate share to pool %d",
                   pool->pool_no);
            free_work(work);
            continue;
        }
        ...
        {
            snprintf(s, sizeof(s),
                     "{\"params\": [\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%d\"], \"id\": %d, \"method\": \"mining.submit\"}",
                     pool->rpc_user, work->job_id, nonce2hex, work->ntime, noncehex,work->version,sshare->id);
        }
        else
#endif
...
static bool setup_gbt_solo(CURL *curl, struct pool *pool)
{
    ...
    applog(LOG_DEBUG, "Solo mining to valid address: %s", opt_btc_address);
    ret = true;
...
retry_stratum:
    ...
    {
        applog(LOG_DEBUG, "Switching pool %d %s to %s", pool->pool_no, pool->rpc_url, pool->stratum_url);
        if (!pool->rpc_url)
...
static void pool_resus(struct pool *pool)
{
    ...
    if (pool_strategy == POOL_FAILOVER && pool->prio < cp_prio())
        ...
        applog(LOG_DEBUG, "Pool %d %s alive", pool->pool_no, pool->rpc_url);
}
                        ...
                        uint64_t nonce2, uint32_t ntime, uint32_t version)
{
    *work = make_work();
    ...
    memcpy(pool->header_bin, &version, 4);
    cg_wunlock(&pool->data_lock);
...
static void submit_work_async(struct work *work)
{
    ...
    if ( *tmp == 0 )
    {
        applog(LOG_WARNING, "Get A Error Data To Push!DROP!\r\n");
        return;
    }
    ...
    if (opt_benchmark)
    {
        ...
        applog(LOG_DEBUG, "Accepted %s %d benchmark share nonce %08x",
    ...
    {
        if (opt_submit_stale)
            applog(LOG_DEBUG, "Pool %d stale share detected, submitting as user requested", pool->pool_no);
        else if (pool->submit_old)
            applog(LOG_DEBUG, "Pool %d stale share detected, submitting as pool requested", pool->pool_no);
        else
        {
            applog(LOG_DEBUG, "Pool %d stale share detected, discarding", pool->pool_no);
...
{
    forcelog(LOG_DEBUG, "%s %d: invalid nonce - HW error", thr->cgpu->drv->name,
             thr->cgpu->device_id);
...
bool submit_tested_work(struct thr_info *thr, struct work *work)
{
    ...
    {
        applog(LOG_DEBUG, "%s %s %d: Share above target",__FUNCTION__, thr->cgpu->drv->name,
               thr->cgpu->device_id);
        return false;
    }
...
bool submit_nonce_1(struct thr_info *thr, struct work *work, uint32_t nonce, int * nofull)
{
    ...
    if (test_nonce(work, nonce))
    {
        ...
        {
            if(nofull) *nofull = 1;
            applog(LOG_DEBUG, "Share above target");
            return false;
        }
...
retry:
    if (pool_active(pool, false))
    {
        ...
        if (unlikely(first_pool))
            applog(LOG_DEBUG, "Switching to pool %d %s - first alive pool", pool->pool_no, pool->rpc_url);
...
# ifdef __linux
    ...
    if (cnfbuf)
    {
        applog(LOG_DEBUG, "Loaded configuration file %s", cnfbuf);
    ...
    applog(LOG_DEBUG, "Probing for an alive pool");
    probe_pools();
    do
    {
        sleep(1);
    ...
    while (42)
    {
        ...
        if (lagging && !pool_tset(cp, &cp->lagging))
        {
            ...
            if (!pool_localgen(cp) && max_queue < opt_queue)
                applog(LOG_DEBUG, "Increasing queue to %d", ++max_queue);
        }
